...
static void rbtree_insert_fixup(RBRoot *root, Node *node)
{
    ...
    while ((parent = rb_parent(node)) && rb_is_red(parent))
    {
        ...
        if (parent == gparent->left)
        {
            // Case 1条件：叔叔节点是红色
            {
                //解决策略:将当前节点的父节点和叔叔节点涂黑，祖父结点涂红，把当前结点指向祖父节点，从新的当前节点重新开始算法
                Node *uncle = gparent->right;
                if (uncle && rb_is_red(uncle))//如果叔叔节点是红色，父节点也是红色，直接将父节点和叔叔节点均涂为黑色，将祖父涂为红色
                {
                    rb_set_black(uncle);
                    rb_set_black(parent);
                    rb_set_red(gparent);
            ...
            {
                //解决策略:当前节点的父节点做为新的当前节点，以新当前节点为支点左旋，此时情况变成了Case 3
                Node *tmp;
            ...
            //解决策略:父节点变为黑色，祖父节点变为红色，在祖父节点为支点右旋
            rb_set_black(parent);
            rb_set_red(gparent);
...
static void rbtree_delete_fixup(RBRoot *root, Node *node, Node *parent)
{
    ...
    //如果被删除的节点是黑色的，就违背了规则5，从任一节点到其每个叶子的所有路径都包含相同数目的黑色节点（黑节点的数目称为黑高black-height）。
    //因为会少一个黑色节点
    while ((!node || rb_is_black(node)) && node != root->node)//node != root->node表示不是树的唯一节点
    {
        if (parent->left == node)//被删除节点是父节点的左子树
        {
            other = parent->right;
            if (rb_is_red(other))
            {
                // Case 1: x的兄弟w是红色的
                rb_set_black(other);//将父节点和兄弟节点颜色互换即可(也就是将兄弟节点设为黑色，父节点设为红色),然后进行左旋
                rb_set_red(parent);
                rbtree_left_rotate(root, parent);
                other = parent->right;
            }
...
void rbtree_delete(RBRoot *root, Node *node)
{
    ...
    if ( (node->left!=NULL) && (node->right!=NULL) )
    {
        ...
        // 获取后继节点
        replace = replace->right;//我们把被删除元素右支的最左边的节点称之为后继节点（后继元素）,也就是其右支中的最小元素
        while (replace->left != NULL)
            replace = replace->left;

        // "node节点"不是根节点(只有根节点不存在父节点)
        if (rb_parent(node))//然后将后继节点与被删除节点进行交换，注意交换的仅仅是颜色,不是值
        {
        ...
        if (color == BLACK) //当被删除节点为红的时候，直接删除即可，如果为黑色，则还需要调整
